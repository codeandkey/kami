...
namespace neocortex {
	...
	public:
		...
		/**
		 * Returns true if the given castling right is active.
		 * 
		 * @param right Castling right. Must be one of CASTLE_WHITE_K, CASTLE_WHITE_Q, etc.
		 * @return true if 'right' is active, false otherwise.
		 */
		bool has_castle_right(int right);
	private:
		Board board;
	...
	inline bool Position::has_castle_right(int right) {
		return ply.back().castle_rights & (1 << right);
	}
